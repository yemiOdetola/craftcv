        ...
        {({ open }) => (
          <>
            ...
            >
              ...
              <Popover.Button
                className={`-mr-1 ml-auto flex h-8 w-8 items-center justify-center ${open && 'relative z-10'}`}
            ...
            <Popover.Panel className="absolute inset-x-0 top-0 bg-white/95 py-3.5 shadow-sm [@supports(backdrop-filter:blur(0))]:bg-white/80 [@supports(backdrop-filter:blur(0))]:backdrop-blur">
              {sections.map((section, sectionIndex) => (
                <Popover.Button key={section.id} className="flex items-center py-1.5 px-4">
        ...
        >
          {sections.map((section, sectionIndex) => (
            <li key={section.id} className="flex [counter-increment:section]">
              <div className={`flex w-full flex-col items-center justify-center border-b-2 before:mb-2 before:font-mono before:text-sm before:content-[counter(section,decimal-leading-zero)]
